    ...
    async def poll_once(self, host: Host):
        ...
        try:
            if host.os == "linux" and host.ssh:
                ...
                # (Se puede ampliar similar a Linux)
                cli = WinRMClient(host.ip, host.winrm)
                # TODO: script windows extendido
                return
        except Exception:
            pass
        ...
        with self.engine.begin() as cx:
            ...
            if extras_payload:
                cx.execute(text("""
                    INSERT INTO extras(host_id, ts, payload) VALUES (:hid, :ts, :p)
...
app = FastAPI(title="Agentless Hub", version="1.1.0")
